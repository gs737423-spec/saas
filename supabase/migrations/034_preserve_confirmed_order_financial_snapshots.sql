...
-- Uma indisponibilidade temporária da origem não pode apagar um snapshot
-- financeiro já confirmado. `unknown` significa falta de evidência nova, não
-- evidência de que a taxa/reembolso confirmado deixou de existir.
...
begin
  ...
  perform pg_catalog.pg_advisory_xact_lock(pg_catalog.hashtextextended(v_company_id || ':' || v_canonical_key, 0));

  insert into public.sales_channels (company_id, canonical_key, display_name, channel_type, status)
  values (
    v_company_id, v_sales_channel,
    coalesce(nullif(p_order->>'sales_channel_display_name', ''), v_sales_channel),
    coalesce(nullif(p_order->>'sales_channel_type', ''), 'marketplace'), 'active'
  ...
  where company_id = v_company_id and connection_id = v_connection_id and external_order_id = v_external_order_id
  for update;

  if v_existing_id is null and v_source_order_id is not null then
    select provider into v_existing_provider
    from public.orders where id = v_source_order_id and company_id = v_company_id for update;
  end if;

  v_order_id := coalesce(v_existing_id, v_source_order_id);
  v_superseded_order_id := case
    when v_existing_id is not null and v_source_order_id is not null and v_existing_id <> v_source_order_id then v_source_order_id
    else null
  end;
  ...
  if v_order_id is null then
    insert into public.orders (
      company_id, connection_id, provider, external_order_id, canonical_order_key, sales_channel,
      source_account, status, total_amount, fee_amount, fee_status, refund_amount, refund_status,
      refund_updated_at, currency, buyer_external_id, ordered_at, source_updated_at,
      analytics_included, unavailable_reason, channel_resolution_status, raw_payload
    ) values (
      v_company_id, v_connection_id, v_provider, v_external_order_id, v_canonical_key, v_sales_channel,
      nullif(p_order->>'source_account', ''), p_order->>'status', (p_order->>'total_amount')::numeric,
      nullif(p_order->>'fee_amount', '')::numeric, coalesce(nullif(p_order->>'fee_status', ''), 'unknown'),
      nullif(p_order->>'refund_amount', '')::numeric, coalesce(nullif(p_order->>'refund_status', ''), 'unknown'),
      nullif(p_order->>'refund_updated_at', '')::timestamptz, nullif(p_order->>'currency', ''), null,
      (p_order->>'ordered_at')::timestamptz, nullif(p_order->>'source_updated_at', '')::timestamptz,
      coalesce((p_order->>'analytics_included')::boolean, true), nullif(p_order->>'unavailable_reason', ''),
    ...
    select
      coalesce(nullif(p_order->>'refund_status', ''), 'unknown') = 'unknown'
      and refund_status in ('known', 'partial')
    into v_preserve_refund
    from public.orders
    ...
    update public.orders set
      ...
      refund_amount = case when v_preserve_refund then refund_amount else nullif(p_order->>'refund_amount', '')::numeric end,
      refund_status = case when v_preserve_refund then refund_status else coalesce(nullif(p_order->>'refund_status', ''), 'unknown') end,
      refund_updated_at = case when v_preserve_refund then refund_updated_at else nullif(p_order->>'refund_updated_at', '')::timestamptz end,
  ...
  if v_superseded_order_id is not null then
    update public.orders set analytics_included = false, unavailable_reason = 'DUPLICATE_CANONICAL_RECONCILED'
    where id = v_superseded_order_id and company_id = v_company_id;
  end if;

  insert into public.order_source_refs (
    company_id, order_id, connection_id, provider, source_account, external_order_id,
    marketplace_order_id, affiliate_id, external_sales_channel, external_marketplace_name,
    channel_key, channel_resolution_status, canonical_order_key, last_seen_at
  ) values (
    v_company_id, v_order_id, v_connection_id, v_provider, nullif(p_source->>'source_account', ''),
    v_external_order_id, nullif(p_source->>'marketplace_order_id', ''), nullif(p_source->>'affiliate_id', ''),
    nullif(p_source->>'external_sales_channel', ''), nullif(p_source->>'external_marketplace_name', ''),
    v_sales_channel, coalesce(nullif(p_source->>'channel_resolution_status', ''), 'resolved'), v_canonical_key, pg_catalog.now()
  ...
  if v_writer_wins then
    delete from public.order_items where company_id = v_company_id and order_id = v_order_id;
    insert into public.order_items (company_id, order_id, external_product_id, sku, title, quantity, unit_price)
    select v_company_id, v_order_id, nullif(item->>'external_product_id', ''), nullif(item->>'sku', ''),
      item->>'title', (item->>'quantity')::integer, (item->>'unit_price')::numeric
    from jsonb_array_elements(coalesce(p_items, '[]'::jsonb)) as item;
  end if;

  return jsonb_build_object('orderId', v_order_id, 'inserted', v_inserted,
