...
-- Leituras específicas para Relatórios e Produto 360. Mantêm a agregação no
-- Postgres e evitam transferir o catálogo inteiro apenas para mostrar poucas
-- linhas ou abrir um único produto.
create index if not exists marketplace_products_company_active_sku_idx
  on public.marketplace_products (company_id, active, sku);
...
as $$
  ...
  low_stock_products as (
    select * from enriched where coalesce(stock, 0) <= 10 order by stock asc nulls first, revenue desc, title asc limit 100
  ),
  metrics as (
    select count(*) filter (where coalesce(stock, 0) <= 10)::integer as low_stock_count,
...
create or replace function public.dashboard_product_lookup(
  p_company_id text,
  p_connection_ids uuid[],
  p_since timestamptz,
  p_until timestamptz,
...
as $$
  with targets as (
    ...
    where p.company_id = p_company_id and p.connection_id = any(p_connection_ids) and p.active = true
      and (
        (p_connection_id is not null and p_external_product_id is not null
          and p.connection_id = p_connection_id and p.external_product_id = p_external_product_id)
        or (p_connection_id is null and p_external_product_id is null and p_sku is not null and p.sku = p_sku)
      )
    limit 20
  ),
  current_sales as (
    select o.connection_id, oi.external_product_id,
      sum(oi.quantity)::bigint as units, sum(oi.quantity * oi.unit_price)::numeric as revenue
    from targets t
    join public.orders o on o.company_id = p_company_id and o.connection_id = t.connection_id
      and o.status = 'paid' and o.analytics_included = true and o.ordered_at >= p_since and o.ordered_at < p_until
    join public.order_items oi on oi.company_id = o.company_id and oi.order_id = o.id and oi.external_product_id = t.external_product_id
    group by o.connection_id, oi.external_product_id
  ),
  previous_sales as (
    select o.connection_id, oi.external_product_id, sum(oi.quantity * oi.unit_price)::numeric as revenue
    from targets t
    join public.orders o on o.company_id = p_company_id and o.connection_id = t.connection_id
      and o.status = 'paid' and o.analytics_included = true and o.ordered_at >= p_previous_since and o.ordered_at < p_since
    join public.order_items oi on oi.company_id = o.company_id and oi.order_id = o.id and oi.external_product_id = t.external_product_id
    group by o.connection_id, oi.external_product_id
  ),
  current_total as (
    select coalesce(sum(oi.quantity * oi.unit_price), 0)::numeric as revenue
    from public.orders o
    join public.order_items oi on oi.company_id = o.company_id and oi.order_id = o.id
    where o.company_id = p_company_id and o.connection_id = any(p_connection_ids)
      and o.status = 'paid' and o.analytics_included = true and o.ordered_at >= p_since and o.ordered_at < p_until
  ...
  rows as (
    select t.*, i.available_quantity as stock, coalesce(cs.units, 0)::bigint as units, coalesce(cs.revenue, 0)::numeric as revenue,
      ps.revenue as previous_revenue, ct.revenue as total_revenue,
      case when t.cost_price is not null and t.price is not null and t.price > 0 then ((t.price - t.cost_price) / t.price) * 100 else null end as margin,
      case when ps.revenue is not null and ps.revenue > 0 then ((coalesce(cs.revenue, 0) - ps.revenue) / ps.revenue) * 100 else null end as trend
    from targets t
    cross join current_total ct
    left join public.marketplace_inventory i on i.company_id = p_company_id and i.connection_id = t.connection_id
      and i.external_product_id = t.external_product_id and i.active = true
    left join current_sales cs on cs.connection_id = t.connection_id and cs.external_product_id = t.external_product_id
    left join previous_sales ps on ps.connection_id = t.connection_id and ps.external_product_id = t.external_product_id
  )
...
revoke all on function public.dashboard_product_lookup(text, uuid[], timestamptz, timestamptz, timestamptz, uuid, text, text) from public, anon, authenticated;
grant execute on function public.dashboard_product_lookup(text, uuid[], timestamptz, timestamptz, timestamptz, uuid, text, text) to service_role;
