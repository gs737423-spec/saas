...
- [x] Tema da plataforma e navegação desktop — 2026-08-12: light mode usa o sistema mineral/graphite/cobalt `#DFDDD8` (app) → `#E9E6E1` (seção) → `#F2EFEB` (card) → `#F7F5F2` (linha) → `#FBFAF8` (elevado), com cabeçalhos de tabela `#D9D4CD`. A Floating Navigation Island mantém 64px e o content width aprovado, com graphite neutro no light (`#33373B` → `#272B2F`) e deep ink no dark (`#20262D` → `#151A20`). O indicador único continua em `transform` (260ms); mobile e `BottomNav` permanecem estruturalmente inalterados. Ver decisão `docs/02-Decisions/2026-08-12 - Hard reset mineral graphite do light mode.md`.
