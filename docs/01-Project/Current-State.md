...
- [x] Light mode Neutral Editorial Enterprise — em 2026-08-13, os tokens de superficie foram migrados de soft slate/blue-gray para uma escala neutral `#DADCD9` (canvas), `#E6E8E5` (section), `#F2F3F1` (card), `#FAFAF8` (raised), com cabecalho de tabela `#D7DAD6`. A TopNav light preserva estrutura e motion, mas usa charcoal neutral `#1E211F` → `#242825` → `#2B302C`; dark mode e arquitetura de viewport nao foram alterados. Ver decisao `docs/02-Decisions/2026-08-13 - Light mode neutral editorial enterprise.md`.
- [x] Shell desktop das rotas `/app`, `/app/marketplaces`, `/app/produtos` e `/app/estoque` — em `md+`, o conteudo operacional ocupa o viewport disponivel abaixo da TopNav, sem rolagem vertical do documento. Dashboard comprime o painel GMV com overflow interno apenas em altura extrema; Marketplaces reserva o fallback de rolagem a sua area analitica; Produtos e Estoque mantem filtros/cabecalhos fixos e rolam somente o viewport das tabelas (incluindo overflow horizontal do Estoque). Mobile preserva fluxo e BottomNav. Implementado em 2026-08-13; validado por `tsc && vite build`.
