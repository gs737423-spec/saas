...
- [x] Tema da plataforma e navegação desktop — 2026-08-12: light mode usa Enterprise Neutral `#E9EDF2` (canvas) → `#F3F5F7` (section) → `#FAFBFC` (card) → `#FFFFFF` (raised/control), com header de tabela `#E3E8ED` e linhas transparentes. A Floating Navigation Island mantém 64px e content width aprovados e ganhou materialidade graphite/blue-steel: gradiente sutil `#1B2530` → `#253441` → `#202C38` no light e a mesma linguagem mais profunda `#141D26` → `#1A2632` → `#17212B` no dark, com dois separadores internos. O indicador único continua em `transform` (260ms); mobile e `BottomNav` permanecem estruturalmente inalterados. Ver decisão `docs/02-Decisions/2026-08-12 - Navbar material graphite blue-steel.md`.
