...
- [x] Tema da plataforma e navegação desktop — 2026-08-12: light mode usa a escala monocromática blue-gray `#DDE5EB` (app) → `#E7EDF2` (seção) → `#F3F6F8` (card) → `#F7F9FA` (destaque), enquanto a Floating Navigation Island é uma âncora dark blue-gray tanto no light quanto no dark. No desktop, ela tem 64px, largura do content width (máximo 1600px com gutters globais), grid marca/navegação/utilidades e indicador compartilhado por `transform` (260ms); mobile e `BottomNav` permanecem inalterados. Ver decisão `docs/02-Decisions/2026-08-12 - Light mode monocromatico e floating nav alinhada.md`.
