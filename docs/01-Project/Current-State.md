...
- [x] Tema da plataforma e navegação desktop — 2026-08-12: light mode usa Enterprise Neutral `#E9EDF2` (canvas) → `#F3F5F7` (section) → `#FAFBFC` (card) → `#FFFFFF` (raised/control), com header de tabela `#E3E8ED` e linhas transparentes. A Floating Navigation Island mantém 64px e content width aprovados, porém é graphite quase sólido `rgba(35,43,52,.94)` no light e deep neutral `#151C24` no dark. O indicador único continua em `transform` (260ms); mobile e `BottomNav` permanecem estruturalmente inalterados. Ver decisão `docs/02-Decisions/2026-08-12 - Enterprise quiet UI do light mode.md`.
