...
- [x] Tema da plataforma e navegação desktop — 2026-08-12: light mode usa a escala monocromática graphite blue `#CDD6DF` (app) → `#DCE3E9` (seção) → `#E9EEF2` (card) → `#F1F4F6` (linha) → `#F6F8F9` (elevado), com bordas e textos de contraste alto. A Floating Navigation Island mantém os mesmos 64px e content width aprovados, mas usa graphite blue escuro sem teal (`#202B38` → `#18232F` no light; `#162230` → `#101A25` no dark). A navegação continua em grid de marca/rotas/utilidades e indicador compartilhado por `transform` (260ms); mobile e `BottomNav` permanecem estruturalmente inalterados. Ver decisão `docs/02-Decisions/2026-08-12 - Correcao cromatica Graphite Blue e light mode.md`.
