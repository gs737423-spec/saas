...
- [x] Tema da plataforma e navegação desktop — 2026-08-12: light mode usa a hierarquia anti-glare azul-acinzentada (`#E8EEF3` app → `#EEF3F7` seção → `#F8FAFC` card → `#FCFDFE` destaque), enquanto a Floating Navigation Island mantém navy glass no dark e frosted blue-gray glass no light. No desktop, a ilha tem 64px e o indicador compartilhado percorre as rotas por `transform` (260ms); mobile e `BottomNav` permanecem inalterados. Ver decisão `docs/02-Decisions/2026-08-12 - Refinamento anti-glare e motion da floating navigation.md`.
